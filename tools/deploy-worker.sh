...
for k in NETHER_KEY HYPIXEL_KEY BORDIC_KEY; do v="${!k}"; [ -n "$v" ] && printf '%s' "$v" | npx --yes wrangler@4 secret put "$k" -c wrangler.toml --name waish-proxy >/dev/null && echo "secret $k set"; done
