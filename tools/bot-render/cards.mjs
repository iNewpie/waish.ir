...
export const SIZE = { user: [1100, 560], check: [1100, 420], stats: [1100, 560], skin: [1100, 560], guild: [1100, 460] };
...
.card { position: relative; width: 100%; height: 100%; padding: 40px 44px 34px; display: flex; flex-direction: column; overflow: hidden;
  background:
...
.head { display: flex; align-items: center; gap: 14px; margin-bottom: 26px; font-family: var(--mono); font-size: 15px; color: var(--muted); }
...
.uuid { font-family: var(--mono); font-size: 15px; color: var(--muted); margin-top: 12px; letter-spacing: .2px; }
...
.tile .k { font-family: var(--mono); font-size: 12px; color: var(--muted); text-transform: uppercase; letter-spacing: .8px; }
.tile .v { font-size: 26px; font-weight: 800; letter-spacing: -.6px; margin-top: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.foot { display: flex; align-items: center; gap: 14px; margin-top: 22px; font-family: var(--mono); font-size: 13px; color: var(--muted); }
...
`;
const page = (cls, inner, w, h) => `<!doctype html><html><head><meta charset="utf-8"><style>${BASE}</style></head><body style="width:${w}px;height:${h}px"><div class="card ${cls}"><div class="grid"></div>${inner}</div></body></html>`;
...
export const CARDS = {
    ...
    <div class="body">
      ...
      <div class="main">
        ...
        <div class="tiles">
          ...
          ${tile('Karma', n(d.karma))}
          ${tile('First seen', d.firstSeen ? esc(d.firstSeen) : '—')}
        </div>
        <div class="badges">${badge(d.seraph)}${badge(d.urchin)}</div>
      </div>
    </div>
  ...
  check: d => page('', `<div class="topline ${d.bad ? 'red' : 'green'}"></div>${head('check', d.ign)}
    <div class="body">
      <div class="figure" style="width:180px"><img src="${body(d.uuid, 'bust')}" alt="" style="max-height:230px"></div>
      <div class="main">
