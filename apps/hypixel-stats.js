...
window.HYP = (() => {
  ...
  const stat = (k, v, hi, color, isCalc) => (v == null || v === '' || v === 'NaN') ? '' : `<div class="stat${isCalc ? ' calc' : ''}${hi ? ' hi' : ''}"><div class="k">${esc(L(k))}</div><div class="v"${color ? ` style="color:${color}"` : ''}>${v}</div></div>`;
  const calc = (k, v, hi, color) => stat(k, v, hi, color, true);
  const grid = cells => {
  ...
  const GAMES = {
    Bedwars: { title: 'BedWars', icon: '🛏️',
      ...
      render(s, pl, P) {
        ...
        return grid([
          star != null ? calc('stars', fmtN(star) + '✫', true, starColor(star)) : '',
          ...
          stat('final kills', N(g('final_kills_bedwars') || 0)), stat('final deaths', N(g('final_deaths_bedwars') || 0)), calc('FKDR', R(g('final_kills_bedwars'), g('final_deaths_bedwars')), true),
    ...
    SkyWars: { title: 'SkyWars', icon: '☁️',
      ...
      render(s, pl, S) {
        ...
        return grid([
          lvl != null ? calc('level', fmtN(lvl) + '⋆', true, swColor(lvl)) : '',
  ...
  function overview() {
    ...
    return `<div class="hy-head">${r ? `<span class="rank" style="color:${r.color}">${esc(r.name)}</span>` : `<span class="tag">Default</span>`}
      ...
      ${section(fa() ? 'خلاصه' : 'Highlights', grid([bwStar != null ? calc('BedWars ' + L('stars'), fmtN(bwStar) + '✫', true, starColor(bwStar)) : '', bw.final_kills_bedwars != null ? calc('BedWars FKDR', R(bw.final_kills_bedwars, bw.final_deaths_bedwars)) : '', sw.skywars_experience != null ? calc('SkyWars ' + L('level'), fmtN(Math.floor(swLevel(sw.skywars_experience))) + '⋆', true, swColor(Math.floor(swLevel(sw.skywars_experience)))) : '', sw.kills != null ? calc('SkyWars KDR', R(sw.kills, sw.deaths)) : '', du.wins != null ? stat('Duels ' + L('wins'), N(du.wins)) : '', du.wins != null ? calc('Duels WLR', R(du.wins, du.losses)) : '', mm.wins != null ? stat('Murder Mystery ' + L('wins'), N(mm.wins)) : '', st.TNTGames && st.TNTGames.wins != null ? stat('TNT Games ' + L('wins'), N(st.TNTGames.wins)) : '', st.Arcade && st.Arcade.coins != null ? stat('Arcade ' + L('coins'), N(st.Arcade.coins)) : '']))}`;
  }
  function guildPanel() {
