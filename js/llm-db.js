...
window.WAISH_DB = {
  ...
  entries: [
    ...
    },
    {
      ...
      answer: {
        en: ["On this computer: Terminal (me) · This PC (files) · Projects folder (LunaMC, ClutchPing, NairoShop) · Social · About me · Music 🎵 · Snake 🐍 · Tetris 🧱 · Mini Minecraft ⛏️ · Calculator · Calendar (Jalali + Gregorian) · User Lookup 🔍 (Minecraft skins, Hypixel stats, guilds, Seraph) · Recycle Bin. Click an icon, use the start menu, or type /open <name> — /apps lists the ids.", "Click any desktop icon, or type /open music, /open snake, /open userlookup, /open calendar… /apps shows every id. The start menu at the bottom-left has them all too."],
        fa: ["روی این کامپیوتر: ترمینال (من) · This PC (فایل‌ها) · پوشهٔ پروژه‌ها (لونا ام‌سی، کلاچ‌پینگ، نایروشاپ) · Social · درباره من · موزیک 🎵 · Snake 🐍 · Tetris 🧱 · ماینکرفت کوچیک ⛏️ · ماشین‌حساب · تقویم (شمسی + میلادی) · User Lookup 🔍 (اسکین ماینکرفت، آمار هایپیکسل، گیلد، Seraph) · سطل بازیافت. روی آیکون کلیک کن، از منوی استارت استفاده کن، یا بنویس /open <name> — /apps آیدی‌ها رو نشون می‌ده.", "روی هر آیکون دسکتاپ کلیک کن، یا بنویس /open music، /open snake، /open userlookup، /open calendar… /apps همهٔ آیدی‌ها رو نشون می‌ده. منوی استارت پایین هم همه رو داره."],
      },
    },
    {
    ...
    },
    {
      ...
      keywords: { en: ['user lookup', 'lookup', 'look up a player', 'lookup a player', 'player lookup', 'search player', 'find a player', 'check a player', 'minecraft player', 'player stats', 'bedwars stats', 'hypixel stats', 'check stats', 'stats', 'uuid', 'name history', 'guild lookup', 'guild', 'skin viewer', '3d skin', 'seraph', 'bordic', 'blacklist', 'sniper', 'cheater check', 'is he a cheater', 'is he blacklisted', 'netherapi', 'nether api', 'hypixel api'], fa: ['user lookup', 'یوزر لوکاپ', 'لوکاپ', 'جستجوی بازیکن', 'جستجوی پلیر', 'بازیکن رو چک کن', 'پلیر رو چک کن', 'آمار بازیکن', 'آمار بدوارز', 'آمار هایپیکسل', 'استتس', 'استت', 'آمار', 'یوآیدی', 'تاریخچه اسم', 'جستجوی گیلد', 'گیلد', 'نمایش اسکین', 'اسکین سه‌بعدی', 'سراف', 'بوردیک', 'بلک‌لیست', 'بلک لیست', 'اسنایپر', 'چیتره', 'چیت میزنه', 'چیت می‌زنه', 'بلک‌لیسته', 'بلک لیسته'] },
      answer: {
        en: ["User Lookup (🔍, or /open userlookup) takes a Minecraft username or UUID and shows: the skin in 3D (drag to rotate), model type, cape, UUID; Hypixel rank, network level, BedWars / SkyWars / Duels stats from Bordic's cache; the player's guild and its tag; whether Seraph has them blacklisted; and Bordic's anti-sniper tags. Three tabs: User Lookup, Skin Lookup (upload your own PNG and preview it), Guild Lookup (by guild name or player).", "Type a name → skin, stats, guild, blacklist status, all in one window. Data sources: Mojang for names/skins/capes, Bordic for Hypixel stats, NetherAPI for guilds, Seraph for the blacklist. Waish is on the Seraph council, so that part matters to him."],
        fa: ["User Lookup (🔍، یا /open userlookup) یه یوزرنیم یا UUID ماینکرفت می‌گیره و نشون می‌ده: اسکین سه‌بعدی (بکش تا بچرخه)، نوع مدل، شنل، UUID؛ رنک هایپیکسل، لول شبکه، آمار بدوارز / اسکای‌وارز / دوئل از کش Bordic؛ گیلد بازیکن و تگش؛ اینکه Seraph بلک‌لیستش کرده یا نه؛ و تگ‌های ضد اسنایپر Bordic. سه تا تب: User Lookup، Skin Lookup (PNG خودت رو آپلود کن و ببین)، Guild Lookup (با اسم گیلد یا بازیکن).", "یه اسم بنویس → اسکین، آمار، گیلد، وضعیت بلک‌لیست، همه توی یه پنجره. منابع: Mojang برای اسم/اسکین/شنل، Bordic برای آمار هایپیکسل، NetherAPI برای گیلد، Seraph برای بلک‌لیست. ویش عضو شورای Seraph هست، برای همین اون بخش براش مهمه."],
      },
      more: { en: ["Seraph is a community blacklist for Hypixel: players reported and verified as cheaters, plus \"caution\" tags. Bordic is an anti-sniper service — it flags accounts that stream-snipe or target specific players. Both are checked by UUID, so name changes don't hide anyone.", "Stats freshness: Bordic only refreshes a player when one of its users meets them in a lobby, so the \"UPDATED\" date under the Hypixel box tells you how old the numbers are. Skins and names always come straight from Mojang, so they're live."], fa: ["Seraph یه بلک‌لیست کامیونیتی برای هایپیکسله: بازیکن‌هایی که به‌عنوان چیتر گزارش و تأیید شدن، به‌علاوهٔ تگ‌های «احتیاط». Bordic یه سرویس ضد اسنایپره — اکانت‌هایی که استریم‌اسنایپ می‌کنن یا بازیکن خاصی رو هدف می‌گیرن رو علامت می‌زنه. هر دو با UUID چک می‌شن، پس تغییر اسم کسی رو مخفی نمی‌کنه.", "تازگی آمار: Bordic فقط وقتی یه بازیکن رو آپدیت می‌کنه که یکی از کاربرهاش توی لابی ببینتش، پس تاریخ «UPDATED» زیر باکس هایپیکسل می‌گه اعداد چقدر قدیمی‌ان. اسکین و اسم همیشه مستقیم از Mojang میان، پس زنده‌ان."] },
    },
    {
      id: 'skin-lookup', emoji: '🧍', title: { en: 'Skin Lookup', fa: 'Skin Lookup' },
      keywords: { en: ['skin lookup', 'skin viewer', 'view skin', 'my skin', 'upload skin', 'upload my skin', 'preview skin', 'skin preview', 'skin png', 'steve', 'alex', 'slim model', 'classic model', 'download skin', 'see a skin', 'check skin', 'change skin'], fa: ['skin lookup', 'نمایش اسکین', 'اسکین رو ببینم', 'اسکینم', 'آپلود اسکین', 'اسکینمو آپلود', 'پیش‌نمایش اسکین', 'پیش نمایش اسکین', 'اسکین png', 'استیو', 'الکس', 'مدل باریک', 'مدل کلاسیک', 'دانلود اسکین', 'یه اسکین ببینم', 'چک اسکین', 'اسکین عوض کنم'] },
      answer: {
        en: ["The Skin Lookup tab in User Lookup shows any skin in 3D: drop a 64×64 skin PNG (or 64×32 legacy) onto it, or type a player's name to pull their current skin. Switch between classic (Steve) and slim (Alex) arms, rotate, zoom, pick idle/walk/run animations, even wear the elytra. There's a Download PNG button for the file.", "Open User Lookup → Skin Lookup tab. Upload your own PNG or load a player by name. 3D preview with Steve/Alex models, animations, and a download button."],
        fa: ["تب Skin Lookup توی User Lookup هر اسکینی رو سه‌بعدی نشون می‌ده: یه PNG اسکین ۶۴×۶۴ (یا ۶۴×۳۲ قدیمی) بنداز روش، یا اسم یه بازیکن رو بنویس تا اسکین فعلیش رو بگیره. بین دست کلاسیک (Steve) و باریک (Alex) سوییچ کن، بچرخون، زوم کن، انیمیشن ایستاده/راه‌رفتن/دویدن رو انتخاب کن، حتی الیترا بپوش. دکمهٔ Download PNG هم برای فایل هست.", "User Lookup رو باز کن → تب Skin Lookup. PNG خودت رو آپلود کن یا یه بازیکن رو با اسم لود کن. پیش‌نمایش سه‌بعدی با مدل‌های Steve/Alex، انیمیشن، و دکمهٔ دانلود."],
      },
    },
    {
